...
func switchProfile(ctx context.Context, args []string) error {
	...
	if err != nil {
		printf("Failed to switch to profile: %v\n", err)
		os.Exit(1)
	}
	...
	for {
		select {
		case <-ctx.Done():
			outln("Timed out waiting for switch to complete.")
			os.Exit(1)
		default:
		}
		st, err := localClient.StatusWithoutPeers(ctx)
		if err != nil {
			outln("Error getting status:", err)
			os.Exit(1)
		}
